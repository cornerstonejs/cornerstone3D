...
addButtonToToolbar({
  ...
  onClick: () => {
    ...
    getAnnotations('Crosshairs', viewports[0].element);
    ...
    viewports.forEach((viewport) => {
      if (viewport instanceof StackViewport) {
        ...
        viewport.resetCamera(true, true, true);
      }
    });
    renderingEngine.render();
    console.log(viewports[0].getCamera());
  },
});
...
async function run() {
  ...
  // const viewports = renderingEngine.getViewports();
  // viewports.forEach((viewport) => {
  //   viewport.setCamera(viewport.getCamera(), true);
  // });
}
