...
saveButton.onclick = convertToRTSS;
//saveButton.onclick = cutPlane;
//saveButton.onclick = runMCubes;
...
async function convertToRTSS() {
  runMSquares();
}

async function runMSquares() {
  const vol = await volumeLoader.loadVolume(segmentationId);
  //console.log(vol);
  const imageVol = await volumeLoader.loadVolume(volumeId);
  ...
  for (let i = 0; i < numSlices; i++) {
    ...
    const reducedSet =
      utilities.contour.mergePoints.removeDuplicatePoints(msOutput);

    if (reducedSet.points && reducedSet.points.length > 0) {
      //console.log(JSON.parse(JSON.stringify(reducedSet)));
      const contours =
        utilities.contour.contourFinder.findContoursFromReducedSet(
          reducedSet.lines,
          reducedSet.points
        );
       ...
       * }
       */
      // Note: will need to be changed to support non-planar contour representation
      ...
      contours.forEach((contour) => {
        ...
        contour.contourPoints.forEach((point) => {
          ContourData.push(reducedSet.points[point]);
        });
