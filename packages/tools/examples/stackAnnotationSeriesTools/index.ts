...
async function run() {
  ...
  toolGroup.setToolPassive(RedactionTool.toolName);
  toolGroup.setToolConfiguration(PlanarFreehandROITool.toolName, {
    calculateStats: true,
  });
  eventTarget.addEventListener(toolsEvents.ANNOTATION_ADDED, (evt) => {
    const { detail } = evt;
    console.log(detail.annotation)
    const length = viewport.getNumberOfSlices()
    annotationFrameRange.setStackFrameRange(detail.annotation, [0,length])
  })
