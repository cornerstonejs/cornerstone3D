...
function onUpload(files) {
  const imageIds = [...files].map((file) => {
    ...
    const instance = metaData.get(MetadataModules.GENERAL_SERIES, imageId);
...
function loadAndViewImages(imageIds) {
  ...
  viewport.setStack(imageIds).then(() => {
    ...
    document.getElementById('studyinstanceuid').innerHTML =
      generalSeriesModule.studyInstanceUID;
