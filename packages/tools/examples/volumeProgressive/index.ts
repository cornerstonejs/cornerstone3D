 ...
 * `mkdicomweb create --alternate ...` invocations. Two commands cover all of
 * them: `create` writes the base store (primary `frames/` keep the source
 * transfer syntax - JPEG-LS lossless for the Juno CT), and `alternates`
 ...
 * ```
 *
 * The rendition directory names below are the `alternates` names: `jls`
 * (full resolution JPEG-LS), `jlsThumbnail` (quarter resolution JPEG-LS),
 * `htj2k` (full resolution lossless HTJ2K) and `htj2kLossy` (full resolution
 * lossy HTJ2K, previously called `lossy`). Serve the result with
 * `dicomwebserver` and load the example with `?useLocal=true`.
 */
const configJLS = {
