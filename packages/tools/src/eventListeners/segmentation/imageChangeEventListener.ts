...
function _imageChangeEventListener(evt) {
  ...
  // this fix is for clearing segmentations when an image/frame changes
  // on a video viewport.
  // why do we need this change?
  //
  // current behaviour:
  // segmentations created on video viewports are not being tied to a frame/image.
  // they are being carried forward to the rest of the frames.
  //
  // expected behaviour:
  // segmentations created on a frame/image should be tied to that frame/image
  // when a frame/image changes, older segmentations should not be persisted/carried forward
  // every frame/image must act like a new canvas ready to be segmented
  if (viewport instanceof VideoViewport) {
    actors.forEach((actor) => {
      ...
      const currentImageId = (
        viewport as Types.IVideoViewport
      ).getCurrentImageId();
    ...
    (viewport as Types.IVideoViewport).render();
    ...
    if (evt.type === Enums.Events.IMAGE_RENDERED) {
      ...
      (viewport as Types.IVideoViewport).element.removeEventListener(
        Enums.Events.IMAGE_RENDERED,
        _imageChangeEventListener as EventListener
      );
    }
  ...
  // the logic from here is on is exclusively for non video viewports
