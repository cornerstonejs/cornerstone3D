import type { Types, WSIGenericViewport } from '@cornerstonejs/core';
import {
  RenderingEngine,
  Enums,
  getRenderingEngine,
...
const viewportId = 'wsiGenericViewport';
const wsiDataId = 'wsi-next:primary';

setTitleAndDescription(
  'WSI GenericViewport',
  'Demonstrates whole slide imaging with the clean GenericViewport WSI API.'
);
...
addButtonToToolbar({
  ...
  onClick: () => {
    ...
    const viewport =
      renderingEngine.getViewport<WSIGenericViewport>(viewportId);
...
async function run() {
  ...
  const viewport = renderingEngine.getViewport<WSIGenericViewport>(viewportId);
