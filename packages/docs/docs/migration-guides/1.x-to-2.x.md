...
//
getSegmentationRepresentations(toolGroupId) --> getRepresentationsForViewport(viewportId)
getToolGroupIdsWithSegmentation(segmentationId) --> getViewportIdsWithSegmentation(segmentationId)
```

Also for the remove it was a wrong name (removeSegmentationsFromToolGroup required representationUIDs).
...
### Segment vs SegmentIndex

There were numerous methods using the term `segment` when they actually referred to `segmentIndex`. Many places used `segmentIndex` and `segment` interchangeably. Now, a `segment` is consistently referred to as a `segment`, and a `segmentIndex` is consistently referred to as a `segmentIndex`.
