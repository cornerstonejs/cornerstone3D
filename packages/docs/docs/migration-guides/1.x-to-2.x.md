...
1. Single source of truth: Previously, data existed in both the image cache and volume cache, leading to synchronization issues between volume and stack viewports. Now, there's only one source of truth: the image cache.

2. Direct GPU upload: For volume rendering, data is now uploaded directly to the GPU instead of being copied to a separate volume cache. This approach allows for more efficient rendering and data manipulation.

3. VoxelManager for data access: Instead of holding a `scalarData` array in the volume object, we now use a `VoxelManager`. This allows manipulation of pixel data at the correct location index or IJK coordinates without the need for a large array buffer.

4. Elimination of SharedArrayBuffer: The new architecture eliminates the need for SharedArrayBuffer, which was a source of frustration for some users due to security context requirements. Each decoded image now goes directly to the GPU 3D texture at the correct size and position.

5. Improved memory management: By avoiding data duplication and large array buffers, memory usage is significantly reduced and more manageable.

6. Enhanced synchronization: With a single source of truth, it's easier to synchronize data across different viewport types (e.g., volume and stack) within the same rendering engine.

These changes result in a more streamlined, efficient, and user-friendly implementation, especially beneficial for large datasets and multi-viewport scenarios.


:::info
The summary is we don't hold any `scalarData` array in the volume object. Instead, we use `VoxelManager` to access the scalar data from individual images.

**Explanation:**
The VoxelManager allows manipulation of pixel data at the correct location index or IJK coordinates without the need for a large array buffer. This approach significantly reduces memory usage and provides more efficient data access, especially for large datasets.
...
4. Available VoxelManager Methods:
   ...
   - `getScalarData()`: Returns the entire scalar data array (if available).
...
6. Iterating over voxels:
   ...
   ```javascript
   voxelManager.forEach(({ value, index, pointIJK, pointLPS }) => {
...
We have removed the VolumeRotateMouseWheelTool and replaced it with StackScrollTool, which now includes options.
...
We have a new segmentation model that is more flexible and easier to use, and
we think it is one of the best things we have done in a long time.
...
2.While it's appropriate for tools to be bound to tool groups, viewport-specific functionalities like segmentation rendering should be the responsibility of individual viewports. So in the second version of our library, we transitioned from tool group-based segmentation representations to viewport-based ones. Now, instead of adding or removing representations to a tool group, users can add them directly to viewports. This change provides much finer control over what each viewport renders. The new approach has proven to be highly effective, and we recognize its significant potential for further improvements.
...
Previously, the segmentation representation was tool group specific, which led to some issues. In the new structure, segmentation representation is viewport specific. It now consists of a segmentationId, a type, and various settings for that segmentation. These settings include active, visible, the colorLUTIndex it's using, and which segments are visible. This new structure, represented by BaseSegmentationRepresentation, provides a more flexible and viewport-oriented approach to managing segmentation representations.


As a result of this change, several functions have been removed or modified. Here's a summary of the changes:
...
#### Removal of Tool Group Specific Events
...
If you were previously using the segmentation configuration functions, you need to update your code to use the new style management functions.
...
const labelmapConfig = getGlobalRepresentationConfig(SegmentationRepresentations.Labelmap);
```
...
setGlobalRepresentationConfig(SegmentationRepresentations.Labelmap, newLabelmapConfig);
```
...
setStyle({ type: SegmentationRepresentations.Labelmap }, {
  renderOutline: true,
  outlineWidth: 2,
});
...
The Color LUT and Segmentation API has undergone significant changes in version 2. These changes aim to improve the API's consistency and provide more flexibility in managing segmentations across viewports.
