...
You should not have any code that references this class, as it is internal, but just in case you do, here is the migration
...
**Migration Guide:**
1. Update all references to request types to use uppercase constants:
   - `interaction` -> `INTERACTION`
   - `thumbnail` -> `THUMBNAIL`
   - `prefetch` -> `PREFETCH`
   - `compute` -> `COMPUTE`
2. Ensure any custom configurations or extensions using these properties are updated accordingly.
...
4. Remove `useWebWorkers` option as web workers are now always used.
5. Remove `decodeConfig` options as they are no longer applicable.
6. Remove separate `webWorkerManager.initialize` call as it's now handled internally.
7. Set `maxWebWorkers` in the configure options instead of a separate config object.
