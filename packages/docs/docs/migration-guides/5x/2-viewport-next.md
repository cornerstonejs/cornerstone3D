...
id: viewport-next
title: Viewport Next
summary: Migration notes for adopting Viewport Next and updating code that depends on legacy viewport classes or removed viewport accessors
---

# Viewport Next Migration Guide

Viewport Next adds new viewport implementations and an optional compatibility
...
## How Viewport Next Is Enabled

You can use Viewport Next directly by requesting a Next viewport type:
...
When `rendering.useViewportNext` is true, legacy viewport requests are remapped
internally:
...
Direct Next viewport types use the new APIs. Remapped legacy viewport types use
...
work reliably with Viewport Next because a `PLANAR_NEXT` viewport can support
...
Code like this is fragile under Viewport Next:
...
If `rendering.useViewportNext` is enabled, a viewport requested as
...
Direct Next viewport code should register logical data ids and then mount them:
...
`rendering.useViewportNext`, prefer keeping the legacy method while migrating:
...
## Direct Next Viewports Use Data APIs

These direct Next viewport types should use `setDataList()`, `setData()`, or
`addData()`:
...
Do not assume direct Next viewports expose the legacy data-loading method names.
...
Viewport Next separates viewport navigation from per-data appearance:
...
3. If enabling `rendering.useViewportNext`, audit `viewport.type` checks for
   legacy types that now run as Next runtime types.
4. For direct Next viewports, replace generic `setDataIds()` and legacy data
