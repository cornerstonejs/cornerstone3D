...
describe('compileExpression', () => {
  ...
  describe('template literals', () => {
    it('builds strings with interpolations', () => {
      ...
      expect(fn({ SeriesDescription: 'AX T1', InstanceNumber: 3 })).toBe(
        'AX T1 #3'
      );
    });
  ...
  describe('helpers and aggregates', () => {
    it('supports defined/includes/startsWith', () => {
      ...
      expect(
        compileExpression("includes(SeriesDescription, 'T1')")({
          SeriesDescription: 'AX T1',
        })
      ).toBe(true);
      expect(
        compileExpression("startsWith(SeriesDescription, 'AX')")({
          SeriesDescription: 'AX T1',
        })
      ).toBe(true);
    });
    ...
    it('evaluates aggregates with per-element scope', () => {
      ...
      expect(
        compileExpression('some(instances, DiffusionBValue != undefined)')(
          context
        )
      ).toBe(true);
      expect(
        compileExpression('every(instances, DiffusionBValue != undefined)')(
          context
        )
      ).toBe(false);
      expect(
        compileExpression('count(instances, DiffusionBValue != undefined)')(
          context
        )
      ).toBe(2);
      expect(
        compileExpression('minOf(instances, InstanceNumber)')(context)
      ).toBe(1);
      expect(
        compileExpression('maxOf(instances, InstanceNumber)')(context)
      ).toBe(3);
      expect(
        compileExpression(
    ...
    it('restores the outer scope after aggregates', () => {
      const fn = compileExpression(
        'some(instances, InstanceNumber > 1) && InstanceNumber === 0'
      );
      expect(
        fn({ instances: [{ InstanceNumber: 5 }], InstanceNumber: 0 })
      ).toBe(true);
    });
  });
  ...
  describe('safety', () => {
    it('rejects prototype-chain access at parse time', () => {
      expect(() => compileExpression('a.__proto__')).toThrow(
        ExpressionSyntaxError
      );
      expect(() => compileExpression('a.constructor')).toThrow(
        ExpressionSyntaxError
      );
      expect(() => compileExpression('a.prototype.x')).toThrow(
        ExpressionSyntaxError
      );
      expect(() => compileExpression('__proto__')).toThrow(
        ExpressionSyntaxError
      );
    });
    ...
    it('rejects non-whitelisted calls at compile time', () => {
      ...
      expect(() => compileExpression('a.toString()')).toThrow(
        ExpressionSyntaxError
      );
    });
    ...
    it('rejects malformed expressions with the source in the message', () => {
      ...
      expect(() => compileExpression('`unterminated')).toThrow(
        ExpressionSyntaxError
      );
    });
