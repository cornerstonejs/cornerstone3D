...
module.exports = {
  ...
  devServer: {
    ...
    // The example index is written straight to disk by
    // build-all-examples-cli.js rather than emitted by rspack, so dev
    // middleware never sees it. Without this, '/' is a 404 when serving.
    // path.resolve is required, not cosmetic: webpack-dev-server rejects a
    // static.directory that looks like a URL, and its Windows-path exemption
    // only matches backslashes, so the posix-slashed 'C:/...' form here would
    // be read as a 'C:' scheme.
    static: { directory: path.resolve(dir) },
    allowedHosts: [
      '127.0.0.1',
      'localhost',
    ],
