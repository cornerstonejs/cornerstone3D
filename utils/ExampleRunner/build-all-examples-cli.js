...
var webpackConfigPath = path.join(__dirname, './webpack-all-examples-AUTOGENERATED.config.js')
...
program
  ...
  .option('--fromRoot', 'A flag to set that this is being run from the root of the repo')
  .parse(process.argv)
...
if (configuration.examples) {
  ...
  configuration.examples.forEach(function (entry) {
    const regexp = entry.regexp ? new RegExp(entry.regexp) : /example\/index.ts$/
