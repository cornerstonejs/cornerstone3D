  ...
  imageIds.forEach((imageId) => {
    ...
    if (instanceMetaData) {
      const NumberOfFrames =
        cornerstoneWADOImageLoader.wadors.metaDataManager.getValue(
          instanceMetaData['00280008']
        );
      if (NumberOfFrames) {
...
function prepareDataset(instanceMetaData) {
  ...
  const shared = instance.SharedFunctionalGroupsSequence.map((sharedInfo) => {
