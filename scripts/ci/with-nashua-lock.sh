...
# runner — across BOTH this repo's workflows (playwright.yml, ohif-downstream.yml)
# AND the OHIF repo. GitHub's `concurrency:` is scoped per-repository and cannot
# coordinate across the two orgs, so the mutex lives on the box's filesystem.
#
# This script can NOT be shared with OHIF (separate repo) — OHIF must carry its
# own copy that points at the SAME lock path (NASHUA_LOCK_FILE default below).
#
...
LOCK="${NASHUA_LOCK_FILE:-/var/tmp/nashua-playwright.lock}"   # MUST match OHIF's copy
