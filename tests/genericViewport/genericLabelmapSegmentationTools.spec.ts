...
const EXAMPLE = 'genericLabelmapSegmentationTools';
...
async function selectSphereBrushAndPaint(page) {
  // Mirror the legacy labelmapsegmentationtools sphere-brush click sequence
  // (axial + sagittal + coronal) so the resulting segmentation overlays
  // align pixel-for-pixel with legacy.
  await page
    .getByRole('combobox')
    .first()
