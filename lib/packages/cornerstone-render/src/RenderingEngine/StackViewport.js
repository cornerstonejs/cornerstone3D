...
class StackViewport extends Viewport {
    ...
    _createVTKImageData(image) {
        const { origin, direction, dimensions, spacing, bitsAllocated, numComps, numVoxels, } = this._getImageDataMetadata(image);
        let pixelArray;
    ...
    _checkVTKImageDataMatchesCornerstoneImage(image, imageData) {
        ...
        if (xSpacing !== image.rowPixelSpacing ||
    ...
    _updateVTKImageDataFromCornerstoneImage(image) {
        const { origin, direction, dimensions, spacing, } = this._getImageDataMetadata(image);
    ...
    _loadImage(imageId, imageIdIndex) {
    ...
    _restoreCameraProps({ focalPoint: prevFocal, position: prevPos, parallelScale: prevScale, }) {
        const renderer = this.getRenderer();
