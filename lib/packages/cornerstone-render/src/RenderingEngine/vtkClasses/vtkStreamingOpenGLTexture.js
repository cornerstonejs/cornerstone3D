...
function vtkStreamingOpenGLTexture(publicAPI, model) {
    ...
    publicAPI.create3DFilterableFromRaw = (width, height, depth, numComps, dataType, data) => {
    ...
    model.fillSubImage3D = (data, frameIndex, bytesPerVoxel, TypedArrayConstructor) => {
        ...
        for (let block = 0; block < normalBlocks; block++) {
            ...
            // Dataview of block
            const dataView = new TypedArrayConstructor(buffer, zOffset + block * multiRowBlockLengthInBytes, multiRowBlockLength);
