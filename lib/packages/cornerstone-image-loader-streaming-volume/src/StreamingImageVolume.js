import { EVENTS, eventTarget, metaData, requestPoolManager, triggerEvent, ImageVolume, cache, loadImage, Utilities, } from '@ohif/cornerstone-render';
...
export default class StreamingImageVolume extends ImageVolume {
    ...
    _prefetchImageIds(priority) {
        ...
        const { imageIds, vtkOpenGLTexture, vtkImageData, metadata, uid: volumeUID, } = this;
        ...
        function successCallback(volume, imageIdIndex, imageId) {
        ...
        interleavedFrames.forEach((frame) => {
            ...
            requestPoolManager.addRequest(sendRequest.bind(this, imageId, imageIdIndex, options), requestType, additionalDetails, priority);
        });
    }
    _addScalingToVolume(suvScalingFactors) {
        if (!this.scaling) {
            this.scaling = {};
        }
    ...
    convertToCornerstoneImage(imageId, imageIdIndex) {
        const { imageIds } = this;
        const { bytesPerImage, pixelsPerImage, windowCenter, windowWidth, numComponents, color, dimensions, spacing, invert, } = this._cornerstoneImageMetaData;
    ...
    _convertToImages() {
        ...
        for (let imageIdIndex = 0; imageIdIndex < numImages; imageIdIndex++) {
            ...
            const imageLodObject = this.convertToCornerstoneImage(imageId, imageIdIndex);
            // 3. Caching the image
