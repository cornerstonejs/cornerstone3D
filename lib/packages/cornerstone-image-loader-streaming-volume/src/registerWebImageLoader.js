...
function loadImage(uri, imageId) {
    ...
    xhr.onprogress = function (oProgress) {
        if (oProgress.lengthComputable) {
            ...
            cornerstone.triggerEvent(cornerstone.eventTarget, 'cornerstoneimageloadprogress', eventData);
        }
    };
 ...
 * ```
 */
function registerWebImageLoader(cs) {
...
function _loadImageIntoBuffer(imageId, options) {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        loadImage(uri, imageId)
            ...
            const pixelDataRGB = new Uint8ClampedArray((pixelDataRGBA.length * 3) / 4);
            let j = 0;
...
//# sourceMappingURL=registerWebImageLoader.js.map